...
#         """
#         파라미터가 0 이상, 최댓값 미만이면 value 에 설정한다.
#         아닐 경우 value 에 0을 설정한다.
#         """
#         # 코드를 쓰세요
...
#         """
#         value 를 1 증가시킨다.
#         카운터의 값 value 가 limit 에 도달하면 value 를 0으로 바꾼 뒤 True 를 리턴한다.
#         value 가 limit 보다 작은 경우 False 를 리턴한다.
#         """
#         # 코드를 쓰세요
#         self.value += 1
...
class Counter:
    ...
    def __init__(self, limit):
        """
        인스턴스 변수 limit(최댓값), value(현재까지 카운트한 값)을 설정한다.
        인스턴스를 생성할 때 인스턴스 변수 limit 만 파라미터로 받고, value 는 초깃값 0으로 설정한다.
        """
        self.limit = limit
        self.value = 0
